defmodule MiniatureSniffleWeb.OrderControllerTest do
  ...
  describe "POST /user/create_order" do
    ...
    test "with valid data, creates an order", %{conn: conn} = context do
      assert Repo.aggregate(Requisition.Order, :count, :id) == 0
      resp_conn = post(conn, "/user/create_order", valid_params(context))
    ...
    test "errors if the changeset is invalid", %{conn: conn} = context do
      resp_conn = post(conn, "/user/create_order", invalid_params(context))
      assert html_response(resp_conn, 200) =~ "something went wrong"
      assert Repo.aggregate(Requisition.Order, :count, :id) == 0
    end
  end

  test "GET /user/create_order2", %{conn: conn} do
    resp_conn =
      conn
      |> assign(:user, %{id: 1, name: "test user"})
      |> get("/user/create_order2")

    assert html_response(resp_conn, 200) =~ "Create Order"
  end

  describe "POST /user/create_order2" do
    setup %{conn: conn} do
      pharmacy =
        %Account.Pharmacy{}
        |> Account.Pharmacy.changeset(%{name: "Valid Pharmacy"})
    ...
    test "with valid data, creates an order", %{conn: conn} = context do
      assert Repo.aggregate(Requisition.Order, :count, :id) == 0
      resp_conn = post(conn, "/user/create_order2", valid_params(context))
      assert html_response(resp_conn, 302)
      assert get_resp_header(resp_conn, "location") == ["/user/create_order2"]
      assert Repo.aggregate(Requisition.Order, :count, :id) == 1
    end

    test "errors if the user is not associated to the location", %{conn: conn} = context do
      resp_conn = post(conn, "/user/create_order2", user_location_mismatch_params(context))
      assert html_response(resp_conn, 200) =~ "something went wrong"
      assert Repo.aggregate(Requisition.Order, :count, :id) == 0
    end

    test "errors if the changeset is invalid", %{conn: conn} = context do
      resp_conn = post(conn, "/user/create_order2", invalid_params(context))
      assert html_response(resp_conn, 200) =~ "something went wrong"
      assert Repo.aggregate(Requisition.Order, :count, :id) == 0
    end
  end
  ...
  defp invalid_params(context) do
    context
    |> valid_params()
